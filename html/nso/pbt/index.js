...
$(function () {
  'use strict';
  createJammerTimerPage();
  createBoxTimerPage(TeamId.Team1);
  createBoxTimerPage(TeamId.Team2);
  createBothTeamsPage();
...
function createJammerTimerPage() {
  pageId = 'JammerTimerPage';
  ...
  //setupJammerTimerPage();  
}

function createBoxTimerPage(teamId) {
  pageId = 'Team'+teamId+'BlockerTimerPage';
...
function createBothTeamsPage() {
  pageId = 'BothTeamsTimerPage';
...
function createSkaterNumber(group, teamId, pos, seatId) {
...
function createTimeEditButton(group, teamId, pos, seatId) {
  var seatTag = getSeatTag(pos,seatId);
  var editBtn = $('<a>')
...
function createBoxTimer(pageId, teamId, pos, seatId) {
  var doBoth = (pageId == 'BothTeamsTimerPage');
  ...
  if(!doBoth) {
    ...
    var block1b = $('<div>')
      .addClass('ui-block-b')
      .appendTo(grid1);
    createSkaterNumber(block1b, teamId, pos, seatId);
  }
  ...
  if(doBoth) {
    ...
    .addClass('ui-block-b')
    .appendTo(grid2a);
    createSkaterNumber(block2ab, teamId, pos, seatId);
  }
  var block2b = $('<div>')
    .addClass('ui-block-b')
    .appendTo(grid2);
  ...
  var block2c = $('<div>')
    .addClass('ui-block-c')
    .appendTo(grid2);
  createTimeEditButton(block2c, teamId, pos, seatId);
  ...
  WS.Register('ScoreBoard.CurrentGame.Team(*).Skater(*).Role',
    function (k, v) {
      if(v == Position.Jammer) {
        var selectId = $('#JammerTimerPageTeam'+k.Team+'DisplayJammer,#BothTeamsTimerPageTeam'+k.Team+'DisplayJammer');
...
WS.Register(
  [
    ...
    function (k, v) {
      ...
      var pageIds = ['Team'+k.Team+'BlockerTimerPage', 'BothTeamsTimerPage'];
  ...
  WS.Register(
    ...
    function (k, v) {
      if(k.BoxSeat != Position.Jammer) {
        var selectId = '#BothTeamsTimerPageTeam'+k.Team+'Select'+k.BoxSeat+',#Team'+k.Team+'BlockerTimerPageTeam'+k.Team+'Select'+k.BoxSeat;
        $(selectId).val(v);
        if(v == "") {
